...
```{r}
sd(df$BALANCE) 
sd(df$BALANCE_FREQUENCY)               
sd(df$PURCHASES)                       
sd(df$ONEOFF_PURCHASES)                
sd(df$INSTALLMENTS_PURCHASES)          
sd(df$CASH_ADVANCE)                    
sd(df$PURCHASES_FREQUENCY)             
sd(df$ONEOFF_PURCHASES_FREQUENCY)      
sd(df$PURCHASES_INSTALLMENTS_FREQUENCY)
sd(df$CASH_ADVANCE_FREQUENCY)          
sd(df$CASH_ADVANCE_TRX)                
sd(df$PURCHASES_TRX)                   
sd(df$CREDIT_LIMIT)                    
sd(df$PAYMENTS)                        
sd(df$MINIMUM_PAYMENTS)                
sd(df$PRC_FULL_PAYMENT)                
sd(df$TENURE)
```
...
Looking at the ranges of the parameters
```{r}
range(df$BALANCE) 
range(df$BALANCE_FREQUENCY)               
range(df$PURCHASES)                       
range(df$ONEOFF_PURCHASES)                
range(df$INSTALLMENTS_PURCHASES)          
range(df$CASH_ADVANCE)                    
range(df$PURCHASES_FREQUENCY)             
range(df$ONEOFF_PURCHASES_FREQUENCY)      
range(df$PURCHASES_INSTALLMENTS_FREQUENCY)
range(df$CASH_ADVANCE_FREQUENCY)          
range(df$CASH_ADVANCE_TRX)                
range(df$PURCHASES_TRX)                   
range(df$CREDIT_LIMIT)                    
range(df$PAYMENTS)                        
range(df$MINIMUM_PAYMENTS)                
range(df$PRC_FULL_PAYMENT)                
range(df$TENURE)
```

```{r}
(0 - mean(df$BALANCE))/ sd(df$BALANCE)
```


Do a scaling check for tha variables
```{r}
scaled_df <- df
...
scaled_df$MINIMUM_PAYMENTS <- scale(df$MINIMUM_PAYMENTS)[, 1]                
scaled_df$PRC_FULL_PAYMENT <- scale(df$PRC_FULL_PAYMENT)[, 1]              
scaled_df$TENURE<- scale(df$TENURE)[, 1]
data.frame (scaled_df)
```


```{r}
sd(scaled_df$BALANCE) 
sd(scaled_df$BALANCE_FREQUENCY)               
sd(scaled_df$PURCHASES)                       
sd(scaled_df$ONEOFF_PURCHASES)                
sd(scaled_df$INSTALLMENTS_PURCHASES)          
sd(scaled_df$CASH_ADVANCE)                    
sd(scaled_df$PURCHASES_FREQUENCY)             
sd(scaled_df$ONEOFF_PURCHASES_FREQUENCY)      
sd(scaled_df$PURCHASES_INSTALLMENTS_FREQUENCY)
sd(scaled_df$CASH_ADVANCE_FREQUENCY)          
sd(scaled_df$CASH_ADVANCE_TRX)                
sd(scaled_df$PURCHASES_TRX)                   
sd(scaled_df$CREDIT_LIMIT)                    
sd(scaled_df$PAYMENTS)                        
sd(scaled_df$MINIMUM_PAYMENTS)                
sd(scaled_df$PRC_FULL_PAYMENT)                
sd(scaled_df$TENURE)
```

```{r}
mean(scaled_df$BALANCE) 
mean(scaled_df$BALANCE_FREQUENCY)               
mean(scaled_df$PURCHASES)                       
mean(scaled_df$ONEOFF_PURCHASES)                
mean(scaled_df$INSTALLMENTS_PURCHASES)          
mean(scaled_df$CASH_ADVANCE)                    
mean(scaled_df$PURCHASES_FREQUENCY)             
mean(scaled_df$ONEOFF_PURCHASES_FREQUENCY)      
mean(scaled_df$PURCHASES_INSTALLMENTS_FREQUENCY)
mean(scaled_df$CASH_ADVANCE_FREQUENCY)          
mean(scaled_df$CASH_ADVANCE_TRX)                
mean(scaled_df$PURCHASES_TRX)                   
mean(scaled_df$CREDIT_LIMIT)                    
mean(scaled_df$PAYMENTS)                        
mean(scaled_df$MINIMUM_PAYMENTS)                
mean(scaled_df$PRC_FULL_PAYMENT)                
mean(scaled_df$TENURE)
```
Reverse Normalization
```{r}
((19043.14-min(df$BALANCE))*((max(scaled_df$BALANCE)-min(scaled_df$BALANCE))/((max(df$BALANCE)-min(df$BALANCE))))) + min(scaled_df$BALANCE)


((12-min(df$TENURE))*((max(scaled_df$TENURE)-min(scaled_df$TENURE))/((max(df$TENURE)-min(df$TENURE))))) + min(scaled_df$TENURE)

```

Check for standered deviation
```{r message=TRUE, warning=TRUE}
range(scaled_df$BALANCE) 
range(scaled_df$BALANCE_FREQUENCY)               
...
range(scaled_df$CASH_ADVANCE_TRX)                
range(scaled_df$PURCHASES_TRX)                   
range(scaled_df$CREDIT_LIMIT)                    
range(scaled_df$PAYMENTS)                        
range(scaled_df$MINIMUM_PAYMENTS)                
range(scaled_df$PRC_FULL_PAYMENT)                
range(scaled_df$TENURE)
```
...
# Compute
library("NbClust")
scaled_df.NbClust <- scaled_df

NbClust(scaled_df, diss=NULL, distance = "euclidean", min.nc=1, max.nc=10, 
            method = "complete", index = "kl") 



clus_df <- scaled_df [c(2:14)]
...
set.seed(123)
fviz_nbclust(clus_df, kmeans, nstart = 25,  method = "gap_stat", nboot = 20)+
